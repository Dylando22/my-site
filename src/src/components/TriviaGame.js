import { Box, Button, Card, CardActionArea, CardContent,  Grid, styled, Typography } from "@mui/material";
import { useState } from "react";
import { useLocation, useNavigate } from "react-router-dom";
import { ArrowBackIos, ArrowForwardIos} from "@mui/icons-material";
import HomeIcon from '@mui/icons-material/Home';

//Buttons press when guessing questions
const TriviaAnswerCard = styled(Card)({
  width:160,
  height:100,
  margin:5,
  padding:5,
...
function TriviaGame(props) {
  ...
  const handleChangeRoundState = (index) => {
    let temp = roundState;
    temp[index] = true;
    setRoundState(temp)
};  
  ...
  return (
    <Box>
      ...
      sx={{
        ...
        <Typography variant="h6" sx={{float:"right", width:"auto", backgroundColor:"primary.main", color:"common.white", padding:"15", marginRight:"10px" }} >Score: {score}</Typography>
        <Typography variant="h6" sx={{float:"right", width:"auto", backgroundColor:"primary.main", color:"common.white", padding:"15", marginRight:"10px" }} >{location.state.playerName}</Typography>
        ...
        <CardContent sx={{
            ...
            <Grid item>
              <TriviaAnswerCard >
                <CardActionArea data-testid='good-click' onClick={ () => {
                  if(location.state.questions[index].Options[0] === location.state.questions[index].Answer
                    && !roundState[index]){
                    ...
                    setAnswerText(`Incorrect, the correct answer is -- ${location.state.questions[index].Answer}`);
                    handleChangeRoundState(index) 
                  }
                  else{
                    setAnswerText(`Already been played, the correct answer is -- ${location.state.questions[index].Answer}`)
                  }
                }}>
                  <CardContent>       
          ...
          <Grid item>
            <TriviaAnswerCard >
              <CardActionArea data-testid='bad-click' onClick={ () => {
                if(location.state.questions[index].Options[1]  === location.state.questions[index].Answer
                  && !roundState[index]){
                  setAnswerText("Correct! - Go to next Question");
                  handleChangeRoundState(index);
                  setScore(score + 1);
          ...
          <Grid item>
            <TriviaAnswerCard >
              <CardActionArea onClick={ () => {
                if(location.state.questions[index].Options[2]  === location.state.questions[index].Answer
                  && !roundState[index]){
                  setAnswerText("Correct! - Go to next Question");
                  handleChangeRoundState(index);
                  setScore(score + 1);
          ...
          <Grid item>
            <TriviaAnswerCard >
              <CardActionArea onClick={ () => {
                if(location.state.questions[index].Options[3]  === location.state.questions[index].Answer
                  && !roundState[index]){
                  setAnswerText("Correct! - Go to next Question");
                  handleChangeRoundState(index);
                  setScore(score + 1); 
      ...
      <Typography variant="h5" sx={{color:"common.black", backgroundColor:"background.paper", borderRadius:"12px" }}>{answerText}</Typography>
    </CardContent>
  </Card>
  <Box display="flex" justifyContent="center" padding={1} width="100%" >
    <Button variant="contained" sx={{marginRight:"30px"}} onClick={() => {navigate("/game-library/trivia-home");}}>
      <HomeIcon />
    </Button>
    ...
    <Button sx={{marginRight:"30px"}}  variant="contained" onClick={() => {
      if(index < QuestionNumber - 1 ){
        setIndex(index + 1);
        setAnswerText("Waiting for guess....");
        }
        else{
          navigate('/game-library/trivia-game-over',{state: 
            {Score: score, 
